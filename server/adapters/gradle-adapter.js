...
\\
   <AGENT alias="BillingSpecificationManager">\\
      <TRAITLIST>\\
        ...
        // Strip broken signatures from OCI JARs (Tomcat 9.0.100 enforces verification)
        const tomcatLib = `${this.appServerHome}/webapps/ROOT/WEB-INF/lib`;
        const beLib = `${BE_HOME}/lib`;
        for (const libDir of [tomcatLib, beLib]) {
            try {
                const stripped = await this._exec(
                    `count=0; for jar in "${libDir}"/oci-java-sdk-*.jar; do ` +
                    `[ -f "$jar" ] && jar tf "$jar" 2>/dev/null | grep -q '\\.SF$' && ` +
                    `zip -qd "$jar" 'META-INF/*.SF' 'META-INF/*.RSA' 'META-INF/*.DSA' 2>/dev/null && ` +
                    `count=$((count+1)); done; echo $count`, 30000);
                if (stripped && stripped !== '0') {
                    logFn(`Stripped signatures from ${stripped} OCI JAR(s) in ${libDir}`);
                }
            } catch (e) {
                logFn(`WARNING: OCI JAR signature strip failed in ${libDir}: ${e.message}`);
            }
        }

        // Remove duplicate Tomcat JARs (provided by Tomcat itself) from both BEServer/lib and Tomcat webapp lib
        for (const libDir of [beLib, tomcatLib]) {
            try {
                const removed = await this._exec(
                    `count=0; for jar in "${libDir}"/tomcat-*-*.jar; do ` +
                    `[ -f "$jar" ] && rm -f "$jar" && count=$((count+1)); done; echo $count`, 10000);
                if (removed && removed !== '0') {
                    logFn(`Removed ${removed} duplicate Tomcat JAR(s) from ${libDir}`);
                }
            } catch (e) {
                logFn(`WARNING: Tomcat JAR cleanup failed in ${libDir}: ${e.message}`);
            }
        }

        // Sync setenv.sh from config
        await this._syncSetenv(logFn);
    }

    async _syncSetenv(logFn) {
        const appServerCfg = this.config.app_server;
        const agentsCfg = this.config.agents || {};
        const setenvPath = `${this.appServerHome}/bin/setenv.sh`;
        const netfolioHome = appServerCfg.netfolio_home || this.deployTarget;
        const clusterName = agentsCfg.server_key || 'DevPetar';

        const lines = [
            '#!/bin/bash',
            '# Auto-generated by server-status from config.yml',
            `NETFOLIO_HOME_MC2="${netfolioHome}"`,
            `CATALINA_OPTS=" -XX:CompileCommand=exclude,org/apache/catalina/startup/ContextConfig,checkHandlesTypes \\`,
            `-Dakka.log-config-on-start=off \\`,
            `-Dakka.agent.system.base.name=${clusterName} \\`,
            `-Dcom.netfolio.appname=web \\`,
            `-Dcom.netfolio.fullname=web0 \\`,
            `-Dakka.config.file=${appServerCfg.akka_config_file || 'dev-petar-akka.conf'} \\`,
            `-Xmx16g \\`,
            `-XX:MaxDirectMemorySize=4g \\`,
            `-Dnetfolio.home=$NETFOLIO_HOME_MC2/ \\`,
            `-Dworker.agent.xmlfile=$NETFOLIO_HOME_MC2/etc/jrunagents.xml \\`,
            `-Dgw.cms.root=${this.appServerHome}/webapps/ROOT/tiles/html \\`,
            `-Dwhite.labels.enabled=no \\`,
            `-Djava.awt.headless=true \\`,
            `-Dhttps.redirect.forced=no \\`,
            `-Deval.code.used=no \\`,
            `-Drebel.struts2-plugin=true \\`,
            `-Drecalculate.strategies=no \\`,
            `-Dsynchronization.disabled=yes \\`,
            `-Dshow.register.client=yes \\`,
            `-Dextjsversion=6 \\`,
            `-DmanagerPortalFirmCd=74 \\`,
            `-DserviceRequest.cutoffTime=7 \\`,
            `-Dlog4j2.formatMsgNoLookups=true \\`,
            `-Dserver.staging=yes \\`,
            `-Ddeveloper.showSql=false \\`,
            `-Ddevelopment.mode=true \\`,
            `-Djava.rmi.server.hostname=localhost \\`,
            `-DNoLockingDuringOrderProcessing \\`,
            `-DAXIS2_HOME=axis2 \\`,
            `-Dworker.reactorpool.size=80 \\`,
            `-Dhttps.redirect.forced=no"`,
        ...
        logFn('setenv.sh synced from config.yml');
    }
