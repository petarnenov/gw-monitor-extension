...
\\
   <AGENT alias="BillingSpecificationManager">\\
      <TRAITLIST>\\
        ...
        // Remove duplicate Tomcat JARs from BEServer/lib (provided by Tomcat itself)
        try {
            const removed = await this._exec(
                `count=0; for jar in "${beLib}"/tomcat-*-*.jar; do ` +
                `[ -f "$jar" ] && rm -f "$jar" && count=$((count+1)); done; echo $count`, 10000);
            if (removed && removed !== '0') {
                logFn(`Removed ${removed} duplicate Tomcat JAR(s) from BEServer/lib`);
            }
        } catch (e) {
            logFn(`WARNING: Tomcat JAR cleanup failed: ${e.message}`);
        }
    ...
    async _syncSetenv(logFn) {
        ...
        lines.push('# Auto-generated by server-status from config.yml');
        lines.push('# Workaround for Corretto 17.0.18 C2 JIT crash');
        lines.push('CATALINA_OPTS="$CATALINA_OPTS -XX:CompileCommand=exclude,org/apache/catalina/startup/ContextConfig,checkHandlesTypes"');

        if (appServerCfg.netfolio_home) {
            lines.push(`CATALINA_OPTS="$CATALINA_OPTS -Dnetfolio.home=${appServerCfg.netfolio_home}"`);
        }
