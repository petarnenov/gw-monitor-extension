...
function checkPlatformReady(config) {
...
function getAppServerStatus(config) {
    return new Promise((resolve) => {
        const start = Date.now();
        const req = http.get(`http://localhost:${config.app_server.port}/`, { timeout: 5000 }, async (res) => {
            res.resume();
            const elapsed = Date.now() - start;
            const proc = getAppServerProcess();
            const ready = res.statusCode === 200 ? await checkPlatformReady(config) : false;
            resolve({
                http_port: config.app_server.port, http_code: res.statusCode,
                response_ms: elapsed, running: res.statusCode === 200,
                ready,
                process: proc,
                threads: getProcessThreads(proc.pid),
                jvm: getJvmConfig(proc.pid),
                webapps: getDeployedApps(config),
                requests_today: getRequestsToday(config),
            });
        });
        req.on('error', () => {
            const proc = getAppServerProcess();
...
function getDeployedApps(config) {
...
function getRequestsToday(config) {
...
module.exports = { getAppServerStatus, getAppServerProcess, registerRoutes };
